...
checkpoint files generated by the simulation scripts, computes evaluation
metrics, and generates the final publication-ready tables and figures.

The analysis is structured in a two-stage workflow:
1.  Sensitivity Analysis: It first analyzes the results from the sensitivity
    study to determine the optimal set of hyperparameters for the RJMCMC model.
    This produces a summary table and a heatmap visualization.
2.  Main Analysis: Using the optimal hyperparameters, it then processes the
    results from the main simulation study, comparing the proposed RJMCMC
    method against the benchmark methods. This produces the main publication
    summary table and a multi-panel figure.
"""
...
from config import (MAIN_RESULTS_DIR, SENSITIVITY_RESULTS_DIR, PLOTS_DIR,
                    SCENARIOS, T, N_REPLICATIONS, SENSITIVITY_GRID)
...
def hausdorff_distance(A, B):
    ...
    This metric measures the "worst-case" discrepancy between the true and
...
def load_and_process_results(results_dir):
    ...
    This version is memory-efficient as it avoids loading the large raw
    posterior sample arrays from the .npz files.
    """
    all_results_list = []
    ...
    for fname in tqdm(result_files):
        ...
        with np.load(fpath, allow_pickle=True) as data:
            # Load only summary keys, skip large sample arrays to save memory
            res = {
                'k_est': data['k_est'].item(),
        ...
        # Parse metadata from the structured filename
        metadata = {}
    ...
    # Compute evaluation metrics for each run and add them as new columns
...
def analyze_sensitivity_results(df):
    ...
    # Aggregate results across all scenarios for each hyperparameter combination
    ...
    # Determine Optimal Parameters by maximizing a score that balances k_accuracy and mae_cfr
...
def analyze_main_results(df_main, baseline_params):
    ...
    # Filter for the main simulation runs using the determined optimal parameters
    ...
    benchmark_mask = df_main['method'].isin(['PELT', 'BinSeg', 'rtaCFR'])
    ...
    # Generate the main multi-panel publication figure
    ...
    for i, scenario_name in enumerate(scen_names):
        ...
        rtacfr_scen_df = scen_df[scen_df['method'] == 'rtaCFR']
            ...
            for rep_idx in tqdm(rjmcmc_scen_df['replication'].unique()):
                ...
                if os.path.exists(fpath):
                    with np.load(fpath, allow_pickle=True) as data:
                        ...
                        all_p_t_samples_agg.append(np.vstack([
                            np.mean(p_t_samples, axis=0),
                            np.percentile(p_t_samples, 2.5, axis=0),
                            np.percentile(p_t_samples, 97.5, axis=0)
                        ]))
        ...
        if all_p_t_samples_agg:
            ...
            ax.plot(avg_stats[0, :], color='dodgerblue', label='RJMCMC Mean Estimate')
            ax.fill_between(range(T), avg_stats[1, :], avg_stats[2, :], color='skyblue', alpha=0.4, label='RJMCMC 95% CrI')
        
        if not rtacfr_scen_df.empty:
            avg_rtacfr_p_t = np.mean(np.vstack(rtacfr_scen_df['p_t_hat'].values), axis=0)
            ax.plot(avg_rtacfr_p_t, color='green', linestyle='--', lw=2, label='rtaCFR Mean Estimate')

        if not scen_df.empty:
            ax.plot(scen_df['true_p_t'].iloc[0], color='black', lw=2, label='True CFR')
